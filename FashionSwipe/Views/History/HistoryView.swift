...
struct HistoryView: View {
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Image(systemName: "heart.slash").font(.system(size: 60)).foregroundColor(.gray.opacity(0.3))
            Text("まだLikeした商品がありません").font(.headline).foregroundColor(.secondary)
        }
...
struct ItemGridCell: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .frame(height: 160).clipped().clipShape(RoundedRectangle(cornerRadius: 10))

            Text(item.cleanName).font(.caption).fontWeight(.medium).lineLimit(2)
            Text("¥\(item.itemPrice.formatted())").font(.caption).fontWeight(.bold)
        }
